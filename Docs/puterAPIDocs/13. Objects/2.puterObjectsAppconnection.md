...
#### [](#-code-usessdk-code-boolean-)`usesSDK` (Boolean)
...
#### [](#-code-on-eventname-handler-code-)`on(eventName, handler)`
...
#### [](#-code-off-eventname-handler-code-)`off(eventName, handler)`

Remove an event listener added with `on(eventName, handler)`.

#### [](#-code-postmessage-message-code-)`postMessage(message)`
...
An app has permission to close apps that it has launched with [`puter.ui.launchApp()`](https://docs.puter.com/UI/launchApp).

[](#examples)Examples
---------------------

### [](#interacting-with-another-app)Interacting with another app
...
### [](#single-app-with-multiple-windows)Single app with multiple windows
